use hearth_save_guard::{protect, protect_live};
...
fn run() -> Result<(), String> {
    ...
    if args.get(1).map(String::as_str) != Some("protect") {
        return Err("usage: HearthSaveGuard protect --save <path> --ledger <dir> [--live]".into());
    }
    ...
    while index < args.len() {
        match args[index].as_str() {
            ...
            _ => {
                return Err(
                    "usage: HearthSaveGuard protect --save <path> --ledger <dir> [--live]".into(),
                )
            }
        }
    }
    ...
    let outcome = if live {
        protect_live(&save, &ledger)
    } else {
        protect(&save, &ledger)
    }
    .map_err(|e| e.to_string())?;
    println!(
        "OK current={} ledger_updates={} restored={} world_records={} world_restored={} world_regression_detected={}",
        ...
        usize::from(outcome.world_regression_detected)
    );
    Ok(())
}
