...
///
/// Basemap capture (top-down screenshot of Karvenia_08) is a follow-on; until
...
internal static class MapSpa
{
    ...
    /// <summary>
    /// Hand-tuned to look reasonable at the Bellwright starting area's
...
<div id="wrap">
  ...
  <div id="list"></div>
  <div id="footer">Hearth · <span id="hearth-version">—</span></div>
</div>
<script>
(function () {
  ...
  var versionEl = document.getElementById('hearth-version');
  ...
  function poll() {
    fetch('/api/v1/map/state', { cache: 'no-store' })
      ...
      .then(function (data) {
        state = data;
        if (data.hearth_version) versionEl.textContent = data.hearth_version;
