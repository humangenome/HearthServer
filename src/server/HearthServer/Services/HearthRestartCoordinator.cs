...
/// <summary>
/// Coordinates between operations that need to pause Bellwright (snapshot restore,
/// world wipe, in-place upgrade) and the supervisor loop that keeps it
/// running. A restore acquires the gate, kills Bellwright if it's running, mutates
/// SaveGames, then releases the gate. The supervisor waits on the gate
/// before each relaunch so a new Bellwright doesn't spawn into a half-written
/// world.
///
/// Kill scoping: only kills Bellwright instances whose executable lives under the
/// configured <see cref="HearthServerOptions.GameInstallRoot"/> (or, when that
/// is empty, instances whose command line references the configured
/// <see cref="HearthServerOptions.GameUserDir"/>). A self-hoster running
/// vanilla Bellwright elsewhere on the same machine will not be touched.
/// </summary>
public sealed class HearthRestartCoordinator
{
    private static readonly string[] HearthProcessNames =
    [
        "BellwrightGame-Win64-Shipping",
    ];

    private readonly ILogger<HearthRestartCoordinator> _log;
    private readonly HearthServerOptions _opts;
    private readonly SemaphoreSlim _restoreGate = new(1, 1);

    public HearthRestartCoordinator(ILogger<HearthRestartCoordinator> log, IOptions<HearthServerOptions> opts)
    {
        _log = log;
        _opts = opts.Value;
    }
    ...
    /// <summary>
    /// Kill the Bellwright instances this HearthServer owns. Scoping is strict:
    ...
    /// touched. Unrelated Bellwright processes on the machine — e.g. a customer's
    ...
    public void KillGame(TimeSpan waitForExit)
    {
        ...
        // HearthServer doesn't own the game's lifecycle and GameInstallRoot is
        ...
        foreach (var name in HearthProcessNames)
        {
            Process[] procs;
            ...
            foreach (var p in procs)
            {
                try
                {
                    ...
                    _log.LogInformation("Restoring: killing Bellwright pid={Pid} name={Name}", p.Id, name);
                    p.Kill(entireProcessTree: true);
                    p.WaitForExit((int)waitForExit.TotalMilliseconds);
                }
                catch (Exception ex)
                {
                    _log.LogWarning(ex, "Failed to kill Bellwright pid={Pid}", p.Id);
                }
                finally
                {
                    try { p.Dispose(); } catch { }
    ...
    private void TryKillFromPidFile(string pidFile, TimeSpan waitForExit)
    {
        ...
        // <root>\Bellwright\Binaries\Win64\BellwrightGame-Win64-Shipping.exe).
        ...
        try
        {
            ...
            var isGame = false;
            foreach (var allowed in HearthProcessNames)
            {
            ...
            {
                _log.LogWarning(
                    "TryKillFromPidFile: pid={Pid} now belongs to '{Name}' (not Bellwright); refusing to kill",
                    pid, name);
                return;
            }
            ...
            // which HearthServer has because it runs as the same user
            ...
            {
                _log.LogWarning(
                    "TryKillFromPidFile: pid={Pid} ({Name}) exe={Exe} is NOT under expected instance root {Root}; refusing to kill (likely PID recycled to a different customer's Bellwright)",
                    pid, name, normExe, expectedRoot);
                return;
            }

            _log.LogInformation(
                "FAIL-CLOSED (pid-file): killing Bellwright pid={Pid} name={Name} exe={Exe} root={Root}",
                pid, name, normExe, expectedRoot);
            p.Kill(entireProcessTree: true);
            p.WaitForExit((int)waitForExit.TotalMilliseconds);
        }
    ...
    private static bool OwnsProcess(Process p, string? installRoot, string? userDir)
    {
        ...
        // privileges for processes owned by other users; in HearthServer's
