...
public static partial class SaveSlotNormalizer
{
        ...
        DateTimeOffset? timestamp = null)
    {
        ...
        log?.LogInformation(
            "Bellwright save slots normalized: active {ActiveSource} -> {Canonical}; archived {ArchivedCount} older slot(s){ArchiveSuffix}",
            active.Name,
            CanonicalSaveName,
    ...
    // so Bellwright can boot cleanly on upgrade.
    private static void PurgeStaleBlockers(string saveGamesDir, ILogger? log)
    {
        ...
        {
            log?.LogInformation(
                "Bellwright save slot legacy blockers purged: {Count} zero-byte sentinel slot(s) removed",
                removed);
        }
    }
