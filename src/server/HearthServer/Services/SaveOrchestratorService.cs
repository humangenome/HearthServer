...
public sealed class SaveOrchestratorService : IHostedService, IDisposable
{
    ...
    /// <summary>
    /// Bellwright listen-host startup creates a fresh savegame_N.sav when multiple
    /// slots are present and no explicit "continue slot" is supplied. Hearth
    ...
    private void TryNormalizeSaveSlots()
    {
        try
        {
            ...
            {
                _log.LogInformation("Save slot normalization skipped: owned Bellwright process is already running");
                return;
            }
        ...
        {
            _log.LogWarning(ex, "Save slot normalization failed; Bellwright may create a new save slot on this launch");
        }
    }
    ...
    /// Invoked from RCON ("save snapshot"), the FileSystemWatcher when Bellwright auto-saves,
    ...
    /// so the supervisor relaunches Bellwright with the restored world.
    /// </summary>
        ...
        string zipPath, string requestedBy, string auditAction, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            // future auto-snapshots stop firing until HearthServer restart.
            try { _watcher?.Dispose(); } catch { }
            _watcher = null;
            TryStartFileWatcher();
        ...
        // gate released by `using` — supervisor will relaunch Bellwright on its
        // next loop iteration.
    }
}
