...
/// <summary>
/// Hosted service that listens for Hearth.dll plugin connections on a per-instance
/// Windows named pipe. Handles handshake, frame parsing, dispatch to handlers, and
/// connection lifecycle.
///
/// Single-connection: Hearth expects exactly one game process per instance, so the
            ...
            case FrameType.Handshake:
                {
                    ...
                    // HearthServerRuntime/main.lua at the very start of
                    // its Lua execution, BEFORE HearthAuth runs. Doing
                    // it here on handshake races HearthAuth - the
                    // handshake fires AFTER Hearth.dll loads, which is
                    // AFTER UE4SS Lua mods run, which means HearthAuth
                    ...
                    // HearthAuth either overwrites it with current
            ...
            case FrameType.PlayerJoined:
                {
                    ...
                    _log.LogInformation("Player joined: {User} ({Name}) from {Addr}", pj.HearthUserId, pj.DisplayName, pj.ClientAddress);
                    return connection;
                }
            ...
            case FrameType.PlayerLeft:
                {
                    ...
                    _log.LogInformation("Player left: {User} reason={Reason}", pl.HearthUserId, pl.Reason);
                    return connection;
                }
