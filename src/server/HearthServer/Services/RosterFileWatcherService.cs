...
/// <summary>
/// Polls <c>roster.json</c> next to HearthServer.exe and pushes the
...
/// (<see cref="HearthHttpService"/>) reflect live player counts and names.
///
/// The producer is the server-side <c>HearthRoster</c> UE4SS Lua mod
/// (<c>dist/ue4ss-server/Mods/HearthRoster/Scripts/main.lua</c>). It hooks
...
/// bridge because (a) the Hearth.dll plugin has no UE reflection access
...
public sealed class RosterFileWatcherService : BackgroundService
{
    ...
    public RosterFileWatcherService(ILogger<RosterFileWatcherService> log, PipeServerState state)
    {
        _log = log;
        _state = state;
        // Lua mod writes to <HearthServer dir>\roster.json. HearthServer's
    ...
    {
        public string? HearthUserId { get; set; }
