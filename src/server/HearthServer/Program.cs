...
public static class Program
{
    public static int Main(string[] args)
    {
        ...
        try
        {
            ...
            builder.Services.Configure<HearthServerOptions>(builder.Configuration.GetSection("Hearth"));
            ...
            builder.Services.AddHostedService<HearthProcessSupervisorService>();
            builder.Services.AddHostedService<HearthLogTailService>();
            builder.Services.AddHostedService<HearthHttpService>();
            ...
            var opts = host.Services.GetRequiredService<IOptions<HearthServerOptions>>().Value;
            ...
            Log.Information("Bellwright user dir: {Dir}", opts.GameUserDir);
        ...
        {
            Log.Fatal(ex, "HearthServer terminated unexpectedly");
            return 1;
        }
        finally
        {
    ...
    // appsettings.json (Hearth:Verbose), env vars, and command-line args, plus
    // an explicit HEARTH_VERBOSE env var. Read here (before the host builds) so
    ...
    private static bool ResolveVerbose(string[] args)
    {
        ...
        var env = Environment.GetEnvironmentVariable("HEARTH_VERBOSE");
        ...
        try
        {
            ...
            return config.GetValue("Hearth:Verbose", false);
        }
        catch
        {
    ...
    {
        var hearthVer = HearthVersionInfo.HearthVersion;
        var os = System.Runtime.InteropServices.RuntimeInformation.OSDescription;
        var dotnetVer = Environment.Version.ToString();
        var host = Environment.MachineName;

        Log.Information("==========================================================");
        Log.Information("  Hearth Server v{Version}  (open-source Bellwright dedicated host)", hearthVer);
        Log.Information("  https://github.com/humangenome/Hearth");
        ...
        Log.Information("  build:   detected from host log (see [Bellwright] lines)");
        Log.Information("==========================================================");
    }
}
