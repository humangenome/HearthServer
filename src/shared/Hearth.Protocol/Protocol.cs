...
namespace Hearth.Protocol;

/// <summary>
/// Wire protocol between Hearth.dll (UE4SS plugin loaded into the Bellwright
/// process) and HearthServer.exe (.NET 8 supervisor). Bumped on any
...
public enum FrameType : byte
{
    ...
    // Mod-host plugin API (Phase 3+). Hearth supports two mod surfaces:
    //   1. Server-side native (C++) mods loaded by Hearth.dll inside Bellwright's
    //      process. They observe game events (player join, chat, world tick)
    //      and can call back into UE5 via UE4SS reflection.
    //   2. Server-side scripted (Lua) mods, also loaded by Hearth.dll,
    //      sandboxed via sol2/LuaJIT. Limited surface: game events + a
    //      curated UE5 helper API.
    //   3. Client-side mods, loaded by HearthLauncher into the customer's Bellwright
    //      via a Hearth-installed UE4SS dropper. The server tells the client
...
public record HeartbeatMessage(
    ...
    // Server fails closed (stop Bellwright) when Configured=1 && HookReady=0.
...
public record PlayerJoinedMessage(
    [property: Key(0)] string HearthUserId,
...
public record JoinTicketResponseMessage(
    [property: Key(0)] bool Accepted,
    [property: Key(1)] string? HearthUserId,
...
{
    /// <summary>Server-side C++ native mod. .dll loaded by Hearth.dll inside Bellwright.</summary>
    ServerNative = 1,
    /// <summary>Server-side Lua mod. .lua loaded into Hearth's sol2 sandbox.</summary>
    ServerLua = 2,
    /// <summary>Client-side mod. .pak/.dll installed by HearthLauncher into the customer's Bellwright install.</summary>
    Client = 3,
}
