...
public sealed class SaveSlotNormalizerTests
{
    ...
    public void Normalize_PurgesStaleZeroByteBlockersFromPriorVersions()
    {
        ...
        // so Bellwright's slot picker doesn't end up in CreateNewGameFailedNoSlots.
    ...
    private sealed class TempDir : IDisposable
    {
        public TempDir()
        {
            Path = System.IO.Path.Combine(System.IO.Path.GetTempPath(), "hearth-save-slot-tests-" + Guid.NewGuid().ToString("N"));
